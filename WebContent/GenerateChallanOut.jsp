...
function saveInvoice()
{
	...
	"&itemDetails="+itemString;
	  ...
	  xhttp.open("POST", "${masterUrl}?a=saveChallanOut", true);
...
<div class="col-sm-3">
 ...
 <div class="col-sm-12">
	   ...
	   <button class="btn btn-primary" id="btnRegister" type="reset" onclick='window.open("?a=showChallans&type=O&txtfromdate=${todaysDate}&txttodate=${todaysDate}&txtfirm=${userdetails.firm_id}")'>Register</button>
		...
		var xhttp = new XMLHttpRequest();
		  xhttp.onreadystatechange = function() 
		  {
		    ...
		    { 	
		    	...
		    	cell5.innerHTML = " <input type='text' class='form-control form-control-sm'  name='txtqtygroup' id='txtqty"+itemId+"' onkeyup='validateQtyAgainstWareHouse("+itemId+");calculateAmount("+itemId+")' onkeypress='digitsOnlyWithDot(event);' value='1'> <input type='hidden' class='form-control form-control-sm'  readonly id='hdnavailableqty"+itemId+"' value="+itemDetails.stockAvailable+">";   	
		    	...
		    	validateQtyAgainstWareHouse(itemId);
...
document.getElementById("divTitle").innerHTML="Generate Challan Out:- "+"${tentativeSerialNo}";
...
if('${invoiceDetails.invoice_id}'!='')
	{
    	...
    	cell3.innerHTML = " <input type='text' class='form-control form-control-sm' id='txtqty"+${item.item_id}+"' onkeyup='calculateAmount(this);validateQtyAgainstWareHouse(${item.item_id})'  onkeypress='digitsOnlyWithDot(event);' value='${item.qty}'> <input type='hidden' readonly id='hdnavailableqty"+${item.item_id}+"'>";   	
...
function generatePdfChallanIn(challanId)
{
	var xhttp = new XMLHttpRequest();
	  ...
	  xhttp.open("GET","?a=generateChallanPDF&challanId="+challanId+"&type=O", false);    
	  xhttp.send();
}
</script>