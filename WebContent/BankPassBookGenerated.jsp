...
<div class="card">
		...
		<br>
              ...
              <div class="row">
				...
				<div class="col-sm-3" align="center">
					<div class="input-group input-group-sm" style="width: 200px;">
	  					<select id="drpfirmid" name="drpfirmid"  class="form-control float-right" onchange="getBanksForThisFirm(this.value,'drpbankid');ReloadFilters()" style="margin-right: 15px;" >
	...
	<div class="col-sm-3" align="center">
					...
					id="drpbankid" name="drpbankid" class="form-control" onchange="ReloadFilters()">
					<option value="-1">---------------Select ----------</option>
				</select>
                  	</div>
				</div>
              ...
              <div class="card-body table-responsive p-0" style="height: 580px;">                
				...
				<c:forEach items="${dailyInvoiceData}" var="item">
					<tr >
						...
						<td>
						<c:if test="${item.customer_name eq null}">
							(Internal Transfer)${item.TransferFirmName}
						</c:if>
						
						<c:if test="${item.customer_name ne null}">
							${item.customer_name}
						</c:if>
...
<script>
  ...
  document.getElementById("divTitle").innerHTML="Bank Pass Book";
...
function ReloadFilters()
{
	...
	{
	 	window.location="?a=generateBankPassBook&bankId="+drpbankid.value+"&txtfromdate="+txtfromdate.value+"&txttodate="+txttodate.value+"&firmId="+drpfirmid.value;
	}
...
getBanksForThisFirm('${firmId}','drpbankid');
	...
	var xhttp = new XMLHttpRequest();
	  xhttp.onreadystatechange = function() 
	  {
	    ...
	    { 		      
	      ...
	      var reqString="<option value='-1'>------Select-----</option>";
