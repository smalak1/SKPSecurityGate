...
function saveInvoice()
{
	...
	"&itemDetails="+itemString;
		  ...
		  xhttp.open("POST", "${masterUrl}?a=savePurchaseInvoice", true);
...
function deleteInvoice(customerId)
{
	...
	var xhttp = new XMLHttpRequest();
	  xhttp.onreadystatechange = function() 
	  {
 	  ...
 	  xhttp.open("GET","?a=deleteInvoice&type=P&invoiceId="+customerId, true);  
...
<c:forEach items="${itemList}" var="item">
			    ...
			    <input type="hidden" id="hdn${item.item_id}" value="${item.item_name}~${item.price}~${item.gst}~${item.size}~${item.color}~${item.product_code}">			    
	   </c:forEach>	   	   	
</datalist>
...
<div class="col-sm-3">
 ...
 <div class="col-sm-12">
	   ...
	   <button class="btn btn-primary" style="display:none" id="generatePDF" type="button" onclick='generateInvoice(${invoiceDetails.invoice_id});'>Generate PDF</button>
	     ...
	     <button class="btn btn-danger" id="btnDelete" onclick="deleteInvoice(${invoiceDetails.invoice_id})">Delete</button>
   </div>
   </div>
		...
		var xhttp = new XMLHttpRequest();
		  xhttp.onreadystatechange = function() 
		  {
		    ...
		    { 	
		    	...
		    	cell2.innerHTML = "<a onclick=window.open('?a=showItemHistory&itemId="+itemId+"') href='#'>"+ itemDetails[0] + "-"+ itemDetails[5] + "-"+itemDetails[4] + "-"+ itemDetails[3]+ "</a>";
		    	...
		    	cell5.innerHTML = " <input type='text' class='form-control form-control-sm'  name='txtqtygroup' id='txtqty"+itemId+"' onkeyup='calculateAmount("+itemId+");' onkeypress='digitsOnlyWithDot(event);' value='1'>";   	
		    	
		    	cell6.innerHTML = "<input type='text' class='form-control form-control-sm' name='txtpricesgroup' id='txtprice"+itemId+"'   onkeyup='calculateAmount("+itemId+");' onkeypress='digitsOnlyWithDot(event);calculateAmount("+itemId+");' value='"+itemDetails[1]+"' >";
		    	
		    	var gstAmount=Number(itemDetails[2]) * Number(itemDetails[1]) / 100 ;
		    	
		    	cell7.innerHTML = "<div class='input-group'>  <input type='text' readonly class='form-control form-control-sm'  name='txtgstamountgroup' id='txtgstamount"+itemId+"' value='"+gstAmount+"'   style='width:50%'>      <input type='text' style='width:50%' class='form-control form-control-sm' onkeyup='calculateAmount("+itemId+")' id='txtgstpercent"+itemId+"' value='"+itemDetails[2]+"'></div>";
		    	cell8.innerHTML = "<input type='text' readonly class='form-control form-control-sm' value='0' id='totalAmount"+itemId+"'  name='txttotalamountgroup' >";   	
...
function calculateAmount(itemId)
{
	...
	var gstpercent=document.getElementById('txtgstpercent'+itemId).value;
	var gstAmount=Number(price)*Number(qty)* Number(gstpercent) /100;
	
	document.getElementById('txtgstamount'+itemId).value=gstAmount;
...
{	
		...
		gstAmount.innerHTML=Number(gstAmountCalculated).toFixed(2);
