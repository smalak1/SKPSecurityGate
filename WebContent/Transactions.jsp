              ...
              <div class="card-body table-responsive p-0" style="height: 580px;">                
				...
				<c:forEach items="${message}" var="item">
					<tr >
						...
						<td>${item.name}</td>
						<td></td><td><button class="btn btn-info" onclick="exportToPDF(${item.payment_id})">Export To PDF</button></td>
						
						<td></td><td><button class="btn btn-danger" onclick="deleteTransaction(${item.payment_id})">Delete</button></td>
...
<script>
  ...
  <c:if test="${param.type eq 'P'}">
  	document.getElementById("divTitle").innerHTML="Transactions (Payments)";
  </c:if>
  
  <c:if test="${param.type eq 'R'}">
  document.getElementById("divTitle").innerHTML="Transactions (Receipts)";
  </c:if>
  ...
  {
	  window.location="?a=showTransactions&type=${param.type}&firmId="+drpfirmId.value+"&txtfromdate="+txtfromdate.value+"&txttodate="+txttodate.value;
  ...
  function deleteTransaction(customerId)
  {
  	...
  	var xhttp = new XMLHttpRequest();
  	  ...
  	  xhttp.open("GET","?a=deleteTransaction&customerid="+customerId, true);    
  	  xhttp.send();
  }
  
  
  function exportToPDF(paymentid)
  {
	
	  
	  var stringtosend="?a=exportPaymentsPDF&payment_id="+paymentid;
